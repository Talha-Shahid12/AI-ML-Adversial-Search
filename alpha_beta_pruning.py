    ...
    def minimax(self, node, depth, alpha, beta, maximizingPlayer):
          ...
          if maximizingPlayer:  # Maximizing player's turn
               ...
               for child, move in node.insert_child('x'):
                    eva, _ = self.minimax(child, depth - 1, alpha, beta, False)
                    if eva > maxEva:
                         maxEva = eva
                         best_move = move
                    alpha = max(eva,alpha)
               ...
               for child, move in node.insert_child('o'):
                    eva, _ = self.minimax(child, depth - 1, alpha, beta, True)
                    if eva < minEva:
                         minEva = eva
                         best_move = move
                    beta = min(eva,beta)
...
alpha = float('-inf')
beta = float('inf')
best_score,best_move = root_node.minimax(root_node, depth,alpha,beta, maximizingPlayer)
